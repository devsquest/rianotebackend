                    ...
                    <form name="add-note-form" id="add-note-form" method="post" action="{{ isset($note) ? url('admin/update-note', ['id' => $note->id]) : url('admin/save-note')}}">
                        ...
                        <div class="card-body">
                            <div class="form-group row m-b-5">
                                ...
                                <div class="col-sm-9">
                                    <input name="note-name" type="text" class="form-control" id="note-name"
                                    value="{{((isset($note) ? $note->name : ''))}}" required>
                                </div>
                            </div>
                            <div class="form-group row m-b-5">
                                <label class="col-sm-3 col-form-label">Status:</label>
                                <div class="col-sm-9">
                                    <input name="note-status" type="checkbox" id="terms4" data-error="Before you wreck yourself"
                                    {{((isset($note) and $note->status == 1) ? 'checked' : '')}}>
                                </div>
                            </div>
                            <div class="form-group row m-b-5">
